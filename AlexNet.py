...
class AlexNet(object):
  ...
  def create(self):
    ...
    self.conv1 = conv(self.X, 11, 11, 96, 4, 4, padding = 'VALID', name = 'conv1', input_channels=3)
    self.pool1 = max_pool(self.conv1, 3, 3, 2, 2, padding = 'VALID', name = 'pool1')
    norm1 = lrn(self.pool1, 2, 2e-05, 0.75, name = 'norm1')
    ...
    norm2 = lrn(self.pool2, 2, 2e-05, 0.75, name = 'norm2')
    ...
    self.flattened = tf.reshape(self.pool5, [-1, shape])
  ...
  def load_initial_weights(self, session):
    ...
    for op_name in weights_dict:
      ...
      if op_name not in self.SKIP_LAYER:
        print('loading '+op_name+' pre-train......')
...
def conv(x, filter_height, filter_width, num_filters, stride_y, stride_x, name,
         padding='SAME', groups=1, input_channels=None):
  """
  ...
  with tf.variable_scope(name) as scope:
    ...
    #weights = tf.get_variable('weights', initializer=tf.truncated_normal([filter_height, filter_width, input_channels/groups, num_filters], dtype=tf.float32, stddev=1e-1))
    ...
    #relu = leaky_relu(bias, alpha=0.5)
    return relu
...
def fc(x, num_in, num_out, name, relu = True):
  with tf.variable_scope(name) as scope:
    ...
    if relu == True:
      ...
      return leaky_relu(act, alpha=0.5)
